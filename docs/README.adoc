...
To change a state the code typically uses one method: `changeFromeToState(String message, State from, State... to)`.
...
As you can see all but the pseudo events reach the event down to the next state (the super-state) on the state stack.
...
to catch any event not handled by any of the sub states.
